...
import WalletBankCard from "./WalletBankCard";
...
const [transactions, setTransactions] = useState<Transaction[]>([
 ...
 const getImageUrl = (type: string): string => {
    ...
    return(
        <div className="wallet-history">
          <div className="wallet-banner">
             <h2 className="wallet-banner-text">Wallet</h2>
          </div>
  ...
  <h3>Last Transaction</h3>
      <button
          className={showAll ? "wallet-show-less-btn" : "wallet-see-all-btn"}
          onClick={() => setShowAll(!showAll)}
        >
          {showAll ? "Show Less" : "See All"} <span className="wallet-arrow-head">›</span>
        </button>
</div>
